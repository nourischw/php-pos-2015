...
function removeMultiItems() {
	var ans = window.confirm("Confirm to delete selected Staff records?");
	if ( !ans ) {
		return false;
	}
...
$(function() {
	...
	$("#list_items").on("click", ".list_details_button", function( event ) {
		...
		window.location = ROOT + "staff/details/" + record_id;
	});
	...
	$("#list_items").on("click", ".list_reset_password_button", function( event ) {
		event.stopPropagation();
		var record_id = $(this).closest(".list_item_row").data("record_id");
